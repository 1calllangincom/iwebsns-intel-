...
if(filemtime("templates/default/modules/event/event_space.html") > filemtime(__file__) || (file_exists("models/modules/event/event_space.php") && filemtime("models/modules/event/event_space.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/event/event_space.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
	...
	if(!empty($event_row)){
		...
		if(($event_row['is_pass']=='1' && $event_row['grade']>='1') || $event_row['user_id']==$ses_uid || $is_admin){
			...
			$user_row=api_proxy("event_member_by_uid","status,fellow,template,check_in",$event_id,$ses_uid);
			...
			if($time<$event_row['start_time']){
				...
				if($user_row['check_in']==1){
					$is_doing_check=$ef_langpackage->ef_activity_already_check;
				}else {
					$is_doing_check=$ef_langpackage->ef_activity_already_check_no;
				}
			}
...
//取得活动所属俱乐部，取得俱乐部成员E-mail
	$t_group_members=$tablePreStr."group_members";
	$t_users=$tablePreStr."users";
	$group_id=$event_row['group_id'];
	$sql="select * from $t_users where user_id in (select user_id from $t_group_members where group_id=$group_id)";
	$group_user_rs=$dbo->getRs($sql);
	...
	<div class="evt_box">
        ...
        <div class="evt_cnt">
            <table cellpadding="0" cellspacing="0" width="100%">
                ...
                <tr>
                	<th><?php echo $ef_langpackage->ef_group_name;?>:</th><td><?php echo event_group($dbo,$event_row['group_id']);?></td>
                </tr>
                <tr>
                	<th><?php echo $ef_langpackage->ef_activity_type;?>:</th><td><?php echo event_type($dbo,$event_row['type_id']);?></td>
                </tr>
                <tr>
                	<th><?php echo $ef_langpackage->ef_activity_location;?>:</th><td><?php echo $event_row['location'];?></dd></td></tr>
                <tr>
            ...
            <?php if($is_doing){?>
              ...
              <li><a class="regular-btn" href="javascript:void(0);" onclick="if(confirm('<?php echo $ef_langpackage->ef_confirm_exit;?>')){event_exit(<?php echo $event_id;?>);}"><?php echo $ef_langpackage->ef_exit_activity;?></a></li>
					...
					<?php if($is_self=='Y' && $is_join>=3){?>
						...
						<li><a class="regular-btn" href="mailto:<?php foreach($group_user_rs as $rs){?><?php echo $rs['user_email'];?>;<?php }?>?subject=致<?php echo event_group($dbo,$event_row['group_id']);?>俱乐部所有成员： <?php echo $event_row['title'];?> 活动开始啦！&body=发起人：<?php echo $event_row['user_name'];?>%0d%0a俱乐部：<?php echo event_group($dbo,$event_row['group_id']);?>%0d%0a活动地点：<?php echo $event_row['location'];?>%0d%0a活动时间：<?php echo date('m月d日 H:i',$event_row['start_time']);?> - <?php echo date('m月d日 H:i',$event_row['end_time']);?>%0d%0a报名截止：<?php echo date('m月d日 H:i',$event_row['deadline']);?>%0d%0a活动编号：<?php echo $event_row['event_id'];?>%0d%0a欢迎大家报名%0d%0a报名地址：http://localhost/iweb/iwebsns/modules.php?app=event_quick"><?php echo $ef_langpackage->ef_activity_mail;?></a></li>
						<li><a class="regular-btn" href="modules.php?app=event_messages&event_id=<?php echo $event_id;?>"><?php echo $ef_langpackage->ef_activity_baoming;?></a></li>
					<?php }?>
            </ul>
