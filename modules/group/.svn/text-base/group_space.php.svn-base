...
if(filemtime("templates/default/modules/group/group_space.html") > filemtime(__file__) || (file_exists("models/modules/group/group_space.php") && filemtime("models/modules/group/group_space.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/group/group_space.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
...
<div class="rs_head <?php echo $show_data;?>"><?php echo $g_langpackage->g_event;?></div>
	<?php foreach($group_events as $rs){?>
			<div class="group_user_list">
				<a class="avatar" href="modules.php?app=event_space&event_id=<?php echo $rs["event_id"];?>" target="_blank">
					<img src=<?php echo $rs['poster_thumb'];?> onerror="parent.pic_error(this)" width="50px" height="50px" title="<?php echo $rs['title'];?>" alt="<?php echo $rs['title'];?>" />
				</a>
			  <div><a href="home.php?h=<?php echo $rs['event_id'];?>" target="_blank"><?php echo sub_str($rs['title'],5,true);?></a></div>
			</div>
	<?php }?>
	<div class="rs_head <?php echo $show_data;?>"><?php echo $g_langpackage->g_mangers;?></div>
	<?php foreach($group_managers as $rs){?>
		
			<div class="group_user_list">
				<a class="avatar" href="home.php?h=<?php echo $rs['user_id'];?>" target="_blank">
					<img src=<?php echo $rs['user_ico'];?> onerror="parent.pic_error(this)" width="100px" height="100px" title="<?php echo $rs['user_name'];?>" alt="<?php echo $rs['user_name'];?>" />
				</a>
...
<div class="rs_head <?php echo $show_data;?>"><?php echo $g_langpackage->g_members;?></div>
	...
	<div class="rs_head <?php echo $show_data;?>"><?php echo $g_langpackage->g_photos;?></div>
