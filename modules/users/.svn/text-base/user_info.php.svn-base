...
if(filemtime("templates/default/modules/users/user_info.html") > filemtime(__file__) || (file_exists("models/modules/users/user_info.php") && filemtime("models/modules/users/user_info.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/users/user_info.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
 ...
 require("api/base_support.php");
  ...
  //引入模块公共权限过程文件
	...
	//用户已定义资料
	$sql="select $t_users.wwid from $t_users where $t_users.user_id=$userid";
	$user_row_wwid = $dbo->getRow($sql);
...
<?php }?>
		<form name="form" method="post" action="do.php?act=user_info&is_finish=<?php echo $is_finish;?>" onsubmit="return check_form();">
			...
			<tr>
					<th><?php echo $u_langpackage->u_wwid;?></th>
					<td><?php echo $user_row_wwid['wwid'];?></td>
				</tr>
				<tr>
	...
	<?php if($show_type){?>
		...
		<tr><th><?php echo $u_langpackage->u_wwid;?>：</th><td><?php echo $user_row_wwid['wwid'];?></td></tr>
