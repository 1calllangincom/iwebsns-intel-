...
//引入语言包
	...
	$blog_rs=array();
	$blog_rs=api_proxy("blog_self_by_uid","*",1,'',4);
...
<?php $is_pri=1;?>
<dl class="log_list" title="<?php echo $is_pri ? filt_word($rs["log_title"]): $b_langpackage->b_limit_blog;?>"  id='<?php echo $t_blog;?>:<?php echo $rs['log_id'];?>:<?php echo $rs["privacy"];?>'>
	<dt>
		<strong><a href="modules.php?app=blog&id=<?php echo $rs['log_id'];?>"><?php echo $is_pri ? filt_word($rs["log_title"]): $b_langpackage->b_limit_blog;?></a></strong>
		<?php if(!$is_pri){?>
		<img src='skin/<?php echo $skinUrl;?>/images/user_privacye.gif' />
		<?php }?>
		
	</dt>
	<dd class="log_list_content"><?php echo $is_pri ? sub_str(strip_tags($rs["log_content"]),200):$b_langpackage->b_limit_blog;?></dd>
	<dd>
