...
if(filemtime("templates/default/modules/event/event_search_list.html") > filemtime(__file__) || (file_exists("models/modules/event/event_search_list.php") && filemtime("models/modules/event/event_search_list.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/event/event_search_list.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
