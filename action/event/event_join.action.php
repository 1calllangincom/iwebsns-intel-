...
//获取用户所在省市
$user_row=api_proxy('user_self_by_uid','reside_province,reside_city,wwid',$user_id);
...
if($event_row['verify']==0 || $is_invite['to_user_id']){
	...
	//插入活动成员表
	$sql="insert into $t_event_members (event_id,user_id,user_name,user_sex,user_ico,reside_province,reside_city,dateline,status,fellow,template,wwid) values ($event_id,$user_id,'$user_name','$user_sex','$user_ico','".$user_row['reside_province']."','".$user_row['reside_city']."',".time().",2,'$fellow','$template','".$user_row['wwid']."')";
