...
if(filemtime("templates/default/modules/event/event_member_manager.html") > filemtime(__file__) || (file_exists("models/modules/event/event_member_manager.php") && filemtime("models/modules/event/event_member_manager.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/event/event_member_manager.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
...
<div class="tabs">
  <ul class="menu">
	...
	<li><a href="modules.php?app=event_upload_photo&event_id=<?php echo $event_id;?>" hidefocus="true"><?php echo $ef_langpackage->ef_upload_photo;?></a></li>
     ...
     <?php if($is_admin=='Y'){?>
    		<li><a href="modules.php?app=event_excel&event_id=<?php echo $event_id;?>" hidefoucs="true"><?php echo $ef_langpackage->ef_event_table;?></a></li>
    		 	<?php }?>
...
<form action="do.php?act=event_check_del&ref=<?php echo $url_this;?>" method="post" target="_self">
<table class="msg_inbox" cellspacing="1" cellpadding="1">
	<thead>
		<tr>
			<td><?php echo $ef_langpackage->ef_wwid;?></td>
			...
			<td><?php echo $ef_langpackage->ef_check;?></td>
		</tr>
  </thead>
    ...
    <tr>
    	...
    	<td>
    	    	<?php if($rs['check_in']==1){?>
    	    	<input type="submit" value=" <?php echo $ef_langpackage->ef_cancel_check;?>  "/> 
    	<?php }?>
    	<?php if($rs['check_in']==0){?>
    	<input type="submit" value=" <?php echo $ef_langpackage->ef_check;?>  "/> 
    	<?php }?>
