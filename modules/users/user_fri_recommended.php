...
if(filemtime("templates/default/modules/users/user_fri_recommended.html") > filemtime(__file__) || (file_exists("models/modules/users/user_fri_recommended.php") && filemtime("models/modules/users/user_fri_recommended.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/users/user_fri_recommended.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
         ...
         <div class="sideitem_body">
			...
			<li>
				...
				<span>
					...
					<img class="" style="cursor:pointer;" onclick="javascript:<?php echo str_replace("{uid}",$val['user_id'],$send_join_js);?>" src="skin/<?php echo $skinUrl;?>/images/add.gif"  title="<?php echo str_replace("{he}",get_TP_pals_sex($val['user_sex']),$mp_langpackage->mp_add_mypals);?>" />
				</span>
			</li>
			<?php }?>
		</ul>
