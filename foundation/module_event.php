...
function event_group_list($event_group_rs,$selectedId)
{
	global $lp_cho;
    $type_list="<select name='group_id' id='group_id' >";
         $seleStr='';
         if($selectedId==""){ $seleStr='selected'; }
         $type_list=$type_list.'<option '.$seleStr.' value="">请选择</option>';
         foreach($event_group_rs as $rs){
           if($selectedId==$rs['group_id']){$seleStr='selected';}else{$seleStr='';}
            $type_list=$type_list.'<option '.$seleStr.' value="'.$rs['group_id'].'">'.$rs['group_name'].'</option>';
         }
   	$type_list=$type_list.'</select>';
   	return $type_list;
}

function event_group_checkbox($event_group_rs,$selectedId)
{
	global $lp_cho;
	$type_list='';
         foreach($event_group_rs as $rs){
            $type_list=$type_list.'<input name="group_id[]" type="checkbox" value="'.$rs['group_id'].'"> '.$rs['group_name'].' </input>'.' ';
         }
   	return $type_list;
}
...
//活动俱乐部
function event_group($dbo,$group_id){
	global $tablePreStr;
	$t_event_group=$tablePreStr."groups";
	$sql="select `group_name` from $t_event_group where group_id=$group_id ";
	$type=$dbo->getRow($sql);
