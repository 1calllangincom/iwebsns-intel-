...
$wwid=intval(get_argp('user_wwid'));
$event_id=intval(get_argp('group_id'));

dbtarget('r', $dbServs);
$dbo=new dbex();
$sql="select user_id,user_name,user_sex,user_ico from isns_users where wwid=$wwid";
$row=$dbo->getRow($sql);
$user_id=intval($row['user_id']);
if(empty($user_id)){
	echo $ea_langpackage->ea_no_wwid;exit;
}
$user_name=$row['user_name'];
$user_sex=$row['user_sex'];
$user_ico=$row['user_ico'];
$sql="select * from isns_event where event_id=$event_id";
$row=$dbo->getRow($sql);
if(empty($row)){
	echo $ea_langpackage->ea_no_event;exit;
}
...
if($event_row['verify']==0 || $is_invite['to_user_id']){
	...
	//插入活动成员表
	$sql="insert into $t_event_members (event_id,user_id,user_name,user_sex,user_ico,reside_province,reside_city,dateline,status,fellow,template,wwid) values ($event_id,$user_id,'$user_name','$user_sex','$user_ico','".$user_row['reside_province']."','".$user_row['reside_city']."',".time().",2,'$fellow','$template',$wwid)";
