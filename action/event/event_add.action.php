...
$group_id = intval(get_argp("group_id"));$group_id = $group_id ? $group_id : 0;
...
//读写分离定义函数
dbtarget('w',$dbServs);
$sql="insert into $t_event (user_id,user_name,title,type_id,province,city,location,start_time,end_time,deadline,detail,limit_num,public,member_num,grade,update_time,template,verify,allow_pic,allow_post,allow_invite,allow_fellow,poster,poster_thumb,group_id) values ($user_id,'$user_name','$title',$type_id,'$province','$city','$location',$start_time,$end_time,$deadline,'$detail',$limit_num,$public,'$member_num','$grade','$time','$template',$verify,$allow_pic,$allow_post,$allow_invite,$allow_fellow,'$poster','$poster_thumb',$group_id)";

if(is_admin_club_up($user_id)=='N'){
	action_return(0,$ea_langpackage->ea_no_permission,'-1');exit;
}
if($dbo->exeUpdate($sql)){
	$event_id = mysql_insert_id();
	$sql = 'INSERT INTO '.$t_event_members.' (event_id,user_id,user_name,user_sex,user_ico,reside_province,reside_city,dateline,status,wwid) VALUES '."('$event_id','$user_id','$user_name','$user_sex','$user_ico','$reside_province','$reside_city',".$time.",4,$wwid)";
	$dbo->exeUpdate($sql);
	if($public!=2){
