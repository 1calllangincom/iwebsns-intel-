...
if(filemtime("templates/default/modules/group/group_member_manager.html") > filemtime(__file__) || (file_exists("models/modules/group/group_member_manager.php") && filemtime("models/modules/group/group_member_manager.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/group/group_member_manager.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
...
<body id="iframecontent">
    ...
    <div class="tabs">
        <ul class="menu">
            ...
            <li><a href="modules.php?app=group_budget&group_id=<?php echo $group_id;?>" title="<?php echo $g_langpackage->g_budget;?>" target="_blank"><?php echo $g_langpackage->g_budget;?></a></li>
        </ul>
    </div>
...
<table class="msg_inbox" cellspacing="1" cellpadding="1">
	<thead>
		<tr>
			<td><?php echo $g_langpackage->g_wwid;?></td>
			<td><?php echo $g_langpackage->g_m_name;?></td>
			<td><?php echo $g_langpackage->g_sex;?></td>
			<td><?php echo $g_langpackage->g_department;?></td>
			<td><?php echo $g_langpackage->g_factory;?></td>
    ...
    <tr>
    	...
    	<?php foreach($info_c_rs[$rs['user_id']] as $val){?>
    	<?php if($val['input_type']==1){?><td><?php echo $val['info_value'];?></td><?php }?>
    	  <?php }?>
