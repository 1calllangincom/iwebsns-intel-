...
if(filemtime("templates/default/modules/event/event_mine.html") > filemtime(__file__) || (file_exists("models/modules/event/event_mine.php") && filemtime("models/modules/event/event_mine.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/event/event_mine.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
  ...
  //引入模块公共权限过程文件
	...
	$is_admin=is_admin_club_up($url_uid);
...
<body id="iframecontent">
	...
	<?php if($is_admin=='Y'){?><div class="create_button"><a href="modules.php?app=event_info"><?php echo $ef_langpackage->ef_launch_activity;?></a></div>
	<?php }?><?php }?>
	...
	<?php foreach($event_rs as $rs){?>
        ...
        <?php }?>
				...
				<dl class="group_list">
					...
					<dd><?php echo $ef_langpackage->ef_group_name;?>：<?php echo event_group($dbo,$rs['group_id']);?></dd>
