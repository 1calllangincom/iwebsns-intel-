...
if(filemtime("templates/default/modules/group/group_info_manager.html") > filemtime(__file__) || (file_exists("models/modules/group/group_info_manager.php") && filemtime("models/modules/group/group_info_manager.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/group/group_info_manager.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
...
function textarea_sub(){
	...
	if(group_resume.length>500){
		parent.Dialog.alert("<?php echo $g_langpackage->g_fill_500_characters;?>");
		return;
	}
	if(gonggao.length>500){
		parent.Dialog.alert("<?php echo $g_langpackage->g_fill_500_characters;?>");
		return;
	}
}
...
<body id="iframecontent">
    ...
    <div class="tabs">
        <ul class="menu">
            ...
            <li><a target="_blank" href="modules.php?app=group_budget&group_id=<?php echo $group_id;?>" title="<?php echo $g_langpackage->g_budget;?>"><?php echo $g_langpackage->g_budget;?></a></li>
        </ul>
    </div>
