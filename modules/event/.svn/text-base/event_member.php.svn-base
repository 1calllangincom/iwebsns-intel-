...
if(filemtime("templates/default/modules/event/event_member.html") > filemtime(__file__) || (file_exists("models/modules/event/event_member.php") && filemtime("models/modules/event/event_member.php") > filemtime(__file__)) ) {
	tpl_engine("default","modules/event/event_member.html",1);
	include(__file__);
}else {
/* debug模式运行生成代码 结束 */
?><?php
...
<div class="pals_list" onmouseover="this.className += ' pals_list_active';" onmouseout="this.className='pals_list';">
	...
	<dl>
	  <dd><?php echo $mp_langpackage->mp_name;?>：<?php echo filt_word($rs["user_name"]);?></dd>
		<dd><?php echo $mp_langpackage->mp_wwid;?>：<?php echo $rs['wwid'];?></dd>
